...
description: "把招标技术指标逐条咬下来的技术标响应文件产线。从招标文件(PDF/Word)提取技术指标 → 编写总分结构响应 → 生成产品功能截图(HTML→1920×1080 后台界面) → 生成 docx(指标批注、SEQ 域图注、多级自动编号、指定格式规范) → WPS 渲染校验页数 → 批注一致性校验。当任务主体是'逐条响应/偏离表/承诺函/指标批注/产品功能截图/技术响应文件成稿'时用本技能，触发词：标书、技术标、投标、技术响应文件、响应文件、偏离表、★▲指标、承诺函、指标批注、招标参数应答、产品功能截图。若任务是从需求或大纲写出整本正文内容并排版(写标书正文、技术方案、暗标、自动目录)，改用 tender-bid-writer。"
---
