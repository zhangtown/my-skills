---
name: kimi-docx
description: Create, edit, repair, and review Word documents (.docx). Supports docx-js document creation, in-place editing of existing .docx files, comments, tracked changes, and DOCX package validation/auto-repair.
---
...
`{skill_path}` is the absolute directory containing the `SKILL.md`:
`$HOME/.pi/agent/skills/kimi-docx`:

```bash
DOCX_SKILL_DIR="$HOME/.pi/agent/skills/kimi-docx"
```
